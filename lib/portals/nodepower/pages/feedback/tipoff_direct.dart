...
class TipOffDirectPage extends StatefulWidget {
  PageContext context;

  TipOffDirectPage({this.context});

  @override
  _TipOffDirectPageState createState() => _TipOffDirectPageState();
}

class _TipOffDirectPageState extends State<TipOffDirectPage> {
  EasyRefreshController _controller;
  List<TipOffDirectFormOR> _forms = [];
  ...
  Future<void> _load() async {
    ...
    var forms;
    if (_filter == 0) {
      forms = await tipOffRemote.pageOpenedDirectForm(_limit, _offset);
    } else if(_filter==1){
      forms = await tipOffRemote.pageClosedDirectForm(_limit, _offset);
    }else{
      forms = await tipOffRemote.pageDirectForm(_limit, _offset);
    }
    if (forms.isEmpty) {
  ...
  List<Widget> _renderItems() {
    ...
    for (var form in _forms) {
      items.add(GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: () {
          widget.context.forward('/feedback/tipoff/direct/flow', arguments: {'form': form}).then((value) {
            if(value==null) {
              return;
            }
            if(_filter==0){
  ...
  _getState(TipOffDirectFormOR form) {
    switch (form.state) {
      case 0:
        return '处理中';
      case -1:
