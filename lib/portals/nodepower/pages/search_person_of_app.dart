...
class PersonOfAppSearchDelegate extends SearchDelegate<String> {
  PageContext context;
  Workgroup workgroup;

  PersonOfAppSearchDelegate(this.context, this.workgroup)
      : super(
...
class SearchResultList extends StatefulWidget {
  ...
  SearchResultList({this.query, this.workgroup, this.context});
...
class _SearchResultListState extends State<SearchResultList> {
  ...
  Future<List<_PersonInfo>> _findPersons() async {
    ...
    await widget.context.ports.callback(
      'get $url netos/1.0',
      restCommand: 'searchPersonsInMyApp',
      headers: {
...
class _PersonCard extends StatefulWidget {
  ...
  _PersonCard({this.context, this.workgroup, this.person});
...
//              ),
              FutureBuilder<bool>(
                future: _existsRecipientInWorkgroup(),
  ...
  Future<bool> _existsRecipientInWorkgroup() async {
    IWorkflowRemote workflowRemote =
    widget.context.site.getService('/remote/org/workflow');
    return await workflowRemote.existsRecipientInWorkgroup(widget.workgroup.code, _person.person);
  }
  _addWorkRecipient() async {
    IWorkflowRemote workflowRemote =
        widget.context.site.getService('/remote/org/workflow');
    await workflowRemote.addWorkRecipient(
        widget.workgroup.code, _person.person);
    if (mounted) {
      setState(() {});
    }
  }

  _removeWorkRecipient() async {
    IWorkflowRemote workflowRemote =
        widget.context.site.getService('/remote/org/workflow');
    await workflowRemote.removeWorkRecipient(
        widget.workgroup.code, _person.person);
    _hitsGz = false;
    _hitsQx = false;
    if (mounted) {
