...
class PlatformFundWenyBill extends StatefulWidget {
  PageContext context;

  PlatformFundWenyBill({this.context});

  @override
  _PlatformFundWenyBillState createState() => _PlatformFundWenyBillState();
}

class _PlatformFundWenyBillState extends State<PlatformFundWenyBill>
  ...
  Future<void> _loadFundIndexer() async {
    ...
    _totalInOfMonth =
        await wenyBillRemote.totalInFundBillOfMonth(_bank.id, _selected);
    _totalOutOfMonth =
        await wenyBillRemote.totalOutFundBillOfMonth(_bank.id, _selected);
  }
...
class _FundBillTabView extends StatefulWidget {
  PageContext context;
  BankInfo bank;
  Stream datePicker;
  ...
  @override
  _FundBillTabViewState createState() => _FundBillTabViewState();
}

class _FundBillTabViewState extends State<_FundBillTabView> {
  List<WenyFundBillOR> _fundBills = [];
  EasyRefreshController _controller;
  ...
  Future<void> _onload() async {
    ...
    var fundBill ;
    if(widget.order>-1) {
      fundBill = await wenyBillRemote.pageFundBillOfMonth(
          widget.bank.id, _selectedDateTime, widget.order, _limit, _offset);
    }else{
      fundBill = await wenyBillRemote.getFundBillOfMonth(
    ...
    _fundBills.addAll(fundBill);
    _offset += fundBill.length;
    if (mounted) {
      setState(() {});
    }
  }
  ...
  @override
  void didUpdateWidget(_FundBillTabView oldWidget) {
  ...
  _getOrderType(WenyFundBillOR bill) {
    switch(bill.order) {
      case 0:
        return '申购单';
      case 1:
