...
class PlatformFreeWenyBill extends StatefulWidget {
  PageContext context;

  PlatformFreeWenyBill({this.context});

  @override
  _PlatformFreeWenyBillState createState() => _PlatformFreeWenyBillState();
}

class _PlatformFreeWenyBillState extends State<PlatformFreeWenyBill>
  ...
  Future<void> _loadFundIndexer() async {
    ...
    _totalInOfMonth =
        await wenyBillRemote.totalInFreeBillOfMonth(_bank.id, _selected);
    _totalOutOfMonth =
        await wenyBillRemote.totalOutFreeBillOfMonth(_bank.id, _selected);
  }
...
class _FreeBillTabView extends StatefulWidget {
  PageContext context;
  BankInfo bank;
  Stream datePicker;
  ...
  @override
  _FreeBillTabViewState createState() => _FreeBillTabViewState();
}

class _FreeBillTabViewState extends State<_FreeBillTabView> {
  List<WenyFreeBillOR> _bills = [];
  EasyRefreshController _controller;
  ...
  Future<void> _onload() async {
    ...
    var bill;
    if (widget.order > -1) {
      bill = await wenyBillRemote.pageFreeBillOfMonth(
          widget.bank.id, _selectedDateTime, widget.order, _limit, _offset);
    } else {
      bill = await wenyBillRemote.getFreeBillOfMonth(
  ...
  @override
  void didUpdateWidget(_FreeBillTabView oldWidget) {
  ...
  _getOrderType(WenyFreeBillOR bill) {
    switch (bill.order) {
      case 0:
        return '申购单';
      case 1:
