...
class StockWenyBill extends StatefulWidget {
  PageContext context;

  StockWenyBill({this.context});

  @override
  _WenyBillStockState createState() => _WenyBillStockState();
}

class _WenyBillStockState extends State<StockWenyBill> {
  WenyBank _bank;
  int _limit = 50, _offset = 0;
  List<StockBillOR> _bills = [];
  ...
  Future<void> _loadBills() async {
    ...
    List<StockBillOR> list =
        await billRemote.pageStockBill(_bank.bank, _limit, _offset);
    if (list.isEmpty) {
      return;
    }
    _bills.addAll(list);
  ...
  _forwardDetails(StockBillOR bill) async{
              ...
              Column(
                ...
                children: <Widget>[
                  Padding(
                    ...
                    child: Text('₩${bill.stock.toStringAsFixed(14)}'),
                  ),
                  Text(
                    '余额 ₩${bill.balance.toStringAsFixed(14)}',
                    style: TextStyle(
                      fontSize: 12,
                      color: Colors.grey[500],
                    ),
