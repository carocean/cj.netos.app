...
class _PayDetailsState extends State<PayDetails> {
  List<PayActivityOR> _activities = [];
  Timer _timer;
  PayOR _record;

  @override
  void initState() {
    _record = widget.context.parameters['pay'];
    if (mounted) {
      setState(() {});
    }
  ...
  Future<void> _loadActivities() async {
    ...
    _activities = await recordRemote.getPayActivies(_record.sn);
  }

  @override
  Widget build(BuildContext context) {
    PayOR record = _record;
    var slivers = <Widget>[
      SliverAppBar(
        pinned: true,
        elevation: 0,
  ...
  Widget _AmountCard(PayOR record) {
    return Container(
  ...
  Widget _DetailsCard(PayOR record) {
    var minWidth = 70.00;
    return Container(
      color: Colors.white,
      child: ListView(
