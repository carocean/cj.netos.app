      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            ...
            widget.context.backward(result: selected);
          },
        ),
        actions: <Widget>[
          _getPopupMenu(),
...
class PersonsView extends StatefulWidget {
  ...
  PersonsView({this.context, this.refresher, this.selectMode});
...
class _PersonsViewState extends State<PersonsView> {
  ...
  @override
  void didUpdateWidget(PersonsView oldWidget) {
    if (oldWidget.selectMode != widget.selectMode ||
        oldWidget.refresher != widget.refresher) {
      oldWidget.refresher = widget.refresher;
      oldWidget.selectMode = widget.selectMode;
      if (mounted) {
        setState(() {});
      }
    }
                ...
                child: Slidable(
                  ...
                  child: CardItem(
                    ...
                    onItemTap: widget.selectMode
                        ? () {
                            _selected[item.official] =
                                !(_selected[item.official] ?? false);
                            if (mounted) setState(() {});
                          }
                        : () {
  ...
  Widget _getTailIcon(Person person) {
    if (!widget.selectMode) {
      return null;
    }
    if (_selected.containsKey(person.official) && _selected[person.official]) {
      return Icon(
        Icons.check,
        size: 18,
        color: Colors.red,
