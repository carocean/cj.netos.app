    ...
    final result = Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          RaisedButton(
            onPressed: () {
              // _controller.document.format(0, doc.length, NotusAttribute.code);
              // var selection = _controller.selection;
              // var a=TextSelection(baseOffset: 0,extentOffset: doc.length);
              var selection=_controller.selection.copyWith(baseOffset: 0,extentOffset: doc.length);
              _controller.updateSelection(selection);

              print('-----${selection.baseOffset} ${selection.extentOffset}');
              if (mounted) {
                setState(() {});
              }
            },
...
class _Mya extends ZefyrToolbarDelegate {
  @override
